...
class CustomerRegistration(http.Controller):
    ...
    def sale_return(self, **kwargs, ):
        ...
        _logger.info('sale_return')
        _logger.info('sale_return')
        _logger.info('sale_return')
        _logger.info('sale_return')
        _logger.info('sale_return')
        _logger.info(kwargs.get('vals'))
        _logger.info(kwargs.get('order_id'))
        if kwargs.get('vals'):
            lines = []
            so_id = False
            ...
            if so_id:
                ...
                stock_picks = so_id.sudo().picking_ids
                _logger.info(stock_picks)
                _logger.info(stock_picks)
                _logger.info(stock_picks)
                _logger.info(stock_picks)
                moves = stock_picks.mapped(
                    'move_line_ids').sudo().filtered(
                    lambda p: p.product_id.id == line['product_id'])
                _logger.info(moves)
                _logger.info(moves)
                _logger.info(moves)
                if moves:
                    moves = moves.sorted('quantity', reverse=True)
