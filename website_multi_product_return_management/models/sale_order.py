...
class SaleOrder(models.Model):
    ...
    return_order_lock = fields.Boolean(string="Return Order Lock",compute="compute_return_order_lock")
    delivered_date = fields.Date(string="Dropship Delivered Date")


    @api.model
    def compute_return_order_lock(self):
        for rec in self:
            if rec.delivered_date:
                if rec.delivered_date >= (date.today() + timedelta(days=7)):
                    rec.return_order_lock = False
                else:
                    rec.return_order_lock = True
            else:
                rec.return_order_lock = True
