...
class SaleReturn(models.Model):
    ...
    def return_confirm(self):
        ...
        if moves:
            ...
            pick = self.env['stock.picking.type'].search([('name','=','Dropship Return')])
            _logger.info(pick)
            _logger.info(pick)
            _logger.info(pick)
            _logger.info(pick)
            ...
            return_pick_wizard = self.env['stock.picking.type'].sudo().create(
                vals)
            ...
            if return_pick:
                _logger.info(return_pick)
                return_pick = self.env['stock.picking'].sudo().browse(
                    return_pick[0])
                _logger.info(return_pick.name)
                _logger.info(return_pick.name)
                _logger.info(return_pick.name)
                _logger.info(return_pick.name)
                ...
                return_pick.write(
                    {'return_order_id': False, 'return_order_pick_id': self.id,
                     'return_order_picking': True,'is_dropship':True})
