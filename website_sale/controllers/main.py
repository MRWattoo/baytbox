...
class WebsiteSale(payment_portal.PaymentPortal):
    ...
    def _prepare_product_values(self, product, category, search, **kwargs):
        ...
        attrib_values = [[int(x) for x in v.split("-")] for v in attrib_list if v]
        attrib_set = {v[1] for v in attrib_values}
        _logger.info('llllllllllllllllllllllllllllllllllllllllllllllllllllll')
        _logger.info('llllllllllllllllllllllllllllllllllllllllllllllllllllll')
        _logger.info('llllllllllllllllllllllllllllllllllllllllllllllllllllll')
        _logger.info('llllllllllllllllllllllllllllllllllllllllllllllllllllll')
        keep = QueryURL(
    ...
    def _get_mandatory_fields_billing(self, country_id=False):
        req = ["name", "email", "street", "city_id","dist_id", "country_id"]
        if country_id:
    ...
    def _get_mandatory_fields_shipping(self, country_id=False):
        req = ["name", "street", "city_id","dist_id", "country_id", "phone"]
        if country_id:
            country = request.env['res.country'].browse(country_id)
            if country.state_required:
                req += ['state_id']
    ...
    def address(self, **kw):
        ...
        if dist:
            dist = int(dist)
        _logger.info(type(city))
        _logger.info(type(dist))
        ...
        if order._is_public_order():
            ...
            elif kw.get('mode') == 'billing': # no mode - refresh without post?
        ...
        if 'submitted' in kw and request.httprequest.method == "POST":
            _logger.info('pppppppppppppppppppppppppppppppppp')
            _logger.info('pppppppppppppppppppppppppppppppppp')
            _logger.info('pppppppppppppppppppppppppppppppppp')
            ...
            if errors:
                ...
                if update_mode == 'new':  # New address
                    ...
                    if address_mode == 'billing':
                        ...
                        if not kw.get('callback') and not order.only_services:
                            kw['callback'] = '/shop/checkout?express=1'
                    elif address_mode == 'shipping':
                        update_values['partner_shipping_id'] = partner_id
                ...
                _logger.info(address_mode)
                _logger.info(address_mode)
                _logger.info(kw['callback'])
                if not errors:
    ...
    def _include_country_and_state_in_address(self, address):
        ...
        _logger.info('sssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss')
        _logger.info('sssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss')
        _logger.info('sssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss')
    ...
    def _get_country_related_render_values(self, kw, render_values):
        ...
        def_dist_id = order.partner_id.dist_id
        if order._is_public_order():
        ...
        city = 'city_id' in values and values['city_id'] != '' and request.env['res.city'].search([('state_id','=',order.partner_id.state_id.id)])
        dist = 'dist_id' in values and values['dist_id'] != '' and request.env['res.district'].search([('city_id','=',def_city_id.id)])
        country = country and country.exists() or def_country_id
        city_id = city and city.exists() or def_city_id
        dist_id = dist and dist.exists() or def_dist_id
        res = {
            'country': country,
            'district_city': city_id,
            'state_district': dist_id,
            'city_id': def_city_id.id,
            'dist_id': def_dist_id.id,
            'country_states': country.get_website_sale_states(mode=mode[1],website=request.website),
            'countries': country.get_website_sale_countries(mode=mode[1], website=request.website),
        }
        _logger.info('_get_country_related_render_values')
        _logger.info('_get_country_related_render_values')
        _logger.info(res)
    ...
    def checkout(self, **post):
        ...
        if redirection:
            return redirection
        _logger.info(post.get('mode'))
        _logger.info(post.get('mode'))
        _logger.info(post.get('mode'))
        _logger.info(post.get('mode'))
        _logger.info('express')
        _logger.info('express')
        _logger.info(post.get('express'))
        _logger.info(post.get('express'))
        _logger.info(post.get('express'))
        if post.get('express'):
    ...
    def country_infos(self, country, mode, **kw):
        return dict(
            ...
            states=[(st.id, st.name, st.code) for st in country.get_website_sale_states(mode=mode,website=request.website)],
